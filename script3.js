...
document
  .getElementById("nom_Field")
  .addEventListener("focusout", focusOutRecheck);
document
  .getElementById("prenom_Field")
  .addEventListener("focusout", focusOutRecheck);
document
  .getElementById("tel_Field")
  .addEventListener("focusout", focusOutRecheck);
document
  .getElementById("email_Field")
  .addEventListener("focusout", focusOutRecheck);
document
  .getElementById("resa1_qt_Field")
  .addEventListener("focusout", focusOutRecheck);
document
  .getElementById("resa2_qt_Field")
  .addEventListener("focusout", focusOutRecheck);
document
  .getElementById("resa3_qt_Field")
  .addEventListener("focusout", focusOutRecheck);
document
  .getElementById("checkbox_Field")
  .addEventListener("focusout", focusOutRecheck);
...
function resaLinesAddEventListener() {
  for (line = 1; line <= nbrOfResaLines; line++) {
    document
      .getElementById("resa" + line + "_select_Field")
      .addEventListener("change", calculationAndDisplayProcess); // calcul des résa dés qu'il y a un changement d'option dans un SELECT
    document
      .getElementById("resa" + line + "_qt_Field")
      .addEventListener("input", calculationAndDisplayProcess); // calcul des résa dés qu'il y a un appui de touche dans un champ de quantité
    document
      .getElementById("resa" + line + "_qt_Field")
      .addEventListener("keydown", qtyKeyCheck); // bloquant tous les caractéres sauf les chiffres et quelques touches utiles dans les champs de quantité
  }
}
...
function selectOptionsBuilder() {
  ...
  selectElements.forEach(function (ElementValue, ElementIndex) {
    selectionOptionsArray.forEach(function (optionValue) {
...
function resaLineCalculationAndDisplay(event) {
  ...
  var qtValue = document.getElementById(
    "resa" + resaNumber + "_qt_Field"
  ).value; // récupération de la quantité

  if (qtValue == 0) {
    // si la quantité est égale à 0, les champs de quantité et de sous-tot de la ligne se vident
    document.getElementById("resa" + resaNumber + "_qt_Field").value = "";
    document.getElementById("resa" + resaNumber + "_soustot_Field").value = "";
  } else {
    // si un chiffre a bien été entré alors le calcul du sous-total pour la ligne en cours est déclenché
    var selectValue = document.getElementById(
      "resa" + resaNumber + "_select_Field"
    ).value; //
    var soustotValue = qtValue * selectValue; //
    document.getElementById("resa" + resaNumber + "_soustot_Field").value =
      soustotValue + "€"; // affichage du sous-total pour la ligne en cours
  }
}
...
function finalCalculationAndDisplay() {
  var sousTotHT = 0;
  for (line = 1; line <= nbrOfResaLines; line++) {
    // addition des 3 sous-totaux
    var checkResa = parseInt(
      document.getElementById("resa" + line + "_soustot_Field").value
    ); // récup du sous-total de la ligne en cours
    if (isNaN(checkResa)) {
  ...
  if (sousTotHT == 0) {
    ...
    document.getElementById("totalTTC_Field").value =
      (sousTotHT * 1.2).toFixed(2).replace(".", ",") + "€"; // affichage du total avec la TVA
  }
}
...
function lastNameCheck(nomPrenomRegex) {
  ...
  if (nom.length < 2) {
    ...
    document.getElementById("nom_Field").classList.add("red_border");
  } else if (nomPrenomRegex.test(nom)) {
    document.getElementById("nom_Field").classList.remove("red_border");
  } else {
    verifMessage =
      "Veuillez ne pas saisir de caractères spéciaux et de nombres dans votre nom de famille" +
      "\n";
    document.getElementById("nom_Field").classList.add("red_border");
  }
  return verifMessage;
...
function firstNameCheck(nomPrenomRegex) {
  ...
  if (prenom.length == 0) {
    document.getElementById("prenom_Field").classList.remove("red_border");
  } else {
    if (nomPrenomRegex.test(prenom)) {
      document.getElementById("prenom_Field").classList.remove("red_border");
    } else {
      verifMessage = "Veuillez saisir votre prénom" + "\n";
      document.getElementById("prenom_Field").classList.add("red_border");
    }
  }
  return verifMessage;
}
...
function telCheck(telRegex) {
  ...
  if (tel.length == 0) {
    document.getElementById("tel_Field").classList.remove("red_border");
  } else {
    if (telRegex.test(tel)) {
      document.getElementById("tel_Field").classList.remove("red_border");
    } else {
      verifMessage = "Vérifiez votre numéro de téléphone" + "\n";
      document.getElementById("tel_Field").classList.add("red_border");
    }
  }
  return verifMessage;
}
...
function mailCheck(mailRegex) {
  ...
  if (mailRegex.test(email)) {
    document.getElementById("email_Field").classList.remove("red_border");
  } else {
    verifMessage =
      "Veuillez saisir votre adresse mail sans oublier le @" + "\n";
    document.getElementById("email_Field").classList.add("red_border");
  }
  return verifMessage;
}
...
function orderCheck() {
  ...
  if (order == 0 || isNaN(order)) {
    ...
    for (let index = 0; index < resa_list.length; index++) {
      document.getElementById(resa_list[index]).classList.add("red_border");
    }
  } else {
    for (let index = 0; index < resa_list.length; index++) {
      document.getElementById(resa_list[index]).classList.remove("red_border");
    }
  }
  return verifMessage;
}
...
function headerBuilder(nom, prenom) {
  ...
  if (prenom.length == 0) {
    ...
    headerBuilder +=
      "subject=Réservation de " + prenom + " " + nom.toUpperCase();
  }
  ...
  for (line = 1; line <= nbrOfResaLines; line++) {
    ...
    if (document.getElementById("resa" + line + "_qt_Field").value > 0) {
      resaLine +=
        ...
        " " +
        selecOptionTranslate(
          document.getElementById("resa" + line + "_select_Field").value
        ) +
        " pour un sous-total de " +
        document.getElementById("resa" + line + "_soustot_Field").value +
        "%0A";
    }
  }
...
function reinit() {
  ...
  for (i = 0; i < fieldsResetList.length; i++) {
    ...
    document
      .getElementById(fieldsResetList[i])
      .classList.remove("validBackground");
    document.getElementById(fieldsResetList[i]).classList.remove("red_border");
    document
      .getElementById(fieldsResetList[i])
      .classList.remove("green_border");
  }
